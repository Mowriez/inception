...
export DBNAME=$(grep DB_NAME /run/secrets/credentials | cut -d '=' -f2)
export DBUSER=$(grep DB_USER /run/secrets/credentials | cut -d '=' -f2)
export DBPASS=$(cat /run/secrets/db_password)
export DBHOST=mariadb
export WP_ADMIN_USER=$(grep WP_ADMIN_USER /run/secrets/credentials | cut -d '=' -f2)
export WP_ADMIN_PASS=$(grep WP_ADMIN_PASS /run/secrets/credentials | cut -d '=' -f2)
export WP_ADMIN_EMAIL=$(grep WP_ADMIN_EMAIL /run/secrets/credentials | cut -d '=' -f2)
export WP_USER=$(grep WP_USER /run/secrets/credentials | cut -d '=' -f2)
export WP_PASS=$(grep WP_PASS /run/secrets/credentials | cut -d '=' -f2)
export WP_EMAIL=$(grep WP_EMAIL /run/secrets/credentials | cut -d '=' -f2)
